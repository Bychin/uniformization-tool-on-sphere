...
std::array<double, 3> ClassicGrid::GetCoordsOfPoint(std::array<double, 2>& point) {
    ...
    return {ci * sj, si * sj, cj}; // X, Y, Z //coords;
}

// GetAnglesOfPoint converts Cartesian coordinates of a point to spherical.
// TODO @classmethod or static
std::array<double, 2> ClassicGrid::GetAnglesOfPoint(std::array<double, 3>& point) {
...
std::tuple<int, std::array<int, 2>> ClassicGrid::GetNextTrapeziumIndices(int prev_side,
                                                                         std::array<int, 2>& prev_trapezium_indices) {
    // std::tuple<int, std::array<int, 2>> result;
...
std::array<int, 2> ClassicGrid::GetUpperTrapeziumIndices(std::array<int, 2>& prev_trapezium_indices) {
    int i = prev_trapezium_indices[0], j = prev_trapezium_indices[1];
    assert(j != 0);
    return {i, j - 1};
}

std::array<int, 2> ClassicGrid::GetLowerTrapeziumIndices(std::array<int, 2>& prev_trapezium_indices) {
    int i = prev_trapezium_indices[0], j = prev_trapezium_indices[1];
    assert(j != trapeziums[i].size() - 1);
    return {i, j + 1};
}

std::array<int, 2> ClassicGrid::GetLeftTrapeziumIndices(std::array<int, 2>& prev_trapezium_indices) {
    int i = prev_trapezium_indices[0], j = prev_trapezium_indices[1];
    if (i == 0) {
        i = trapeziums.size();
    }
...
std::array<int, 2> ClassicGrid::GetRightTrapeziumIndices(std::array<int, 2>& prev_trapezium_indices) {
    int i = prev_trapezium_indices[0], j = prev_trapezium_indices[1];
...
std::tuple<int, std::array<double, 3>> ClassicGrid::ProcessSegment(
    std::array<std::array<int, 2>, 4>& trapezium, int index, int start_side, double iso_value) {
    int end_side = -1;
...
int ClassicGrid::GetTrapeziumIndex(std::array<std::array<int, 2>, 4>& trapezium, double value) {
    int index = 0;
